...
fetch("https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=1", options)
  ...
  .then((response) => {
    ...
    searchButton.addEventListener("click", () => {
      ...
      searchFunction(inputText); // 함수를 사용하여 결과를 필터링하고 표시
    });

    function searchFunction(val = "") {
      movies = response.results.filter((movie) => movie.title.toLowerCase().includes(val));
      cardList.innerHTML = ""; // 기존 카드 지우기
      movies.forEach((a) => {
        // movies를 표시하기 위해 forEach 루프를 여기서 반복
        const img = a["poster_path"]; // 포스터
        const title = a["title"]; // 제목
        const overview = a["overview"]; // 요약 내용
        const voteAverage = a["vote_average"]; // 평점
